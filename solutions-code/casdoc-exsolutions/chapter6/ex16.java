...
public class Program
{
    ...
    /*?
     * Keyword: createAddCommand
     * We define an instance method to act as a command factory. The method `createAddCommand` is used to create a command that adds a show to a given day. The method takes two parameters: a show and a day. The method is implemented as an anonymous class that returns a `Command` object which overrides the `execute()`  and `undo()` methods to define the behavior of the commands to be executed, which is to add a show to a given day using the method `add()` and to undo the add operation using the method `remove()`.
     */
    public Command createAddCommand(Show pShow, Day pDay) {
        return new Command() {
            ...
            /*?
             * Keyword: execute
             * We override the `execute()` method to define the behavior of the command to be executed, which is to add a show to a given day using the method `add()`.
             */
            public void execute() {
                add(pShow, pDay);
            }
            ...
            /*?
             * Keyword: undo
             * We override the `undo()` method to define the behavior of the command to be executed, which is to remove a show from a given day using the method `remove()`. This is used to implement the undo functionality for the add command.
             */
            public void undo() {
                remove(pDay);
            }
    ...
    /*?
     * Keyword: createRemoveCommand
     * We define an instance method to act as a command factory. The method `createRemoveCommand` is used to create a command that removes a show from a given day. The method takes a single parameter: a day. The method is implemented as an anonymous class that returns a `Command` object which overrides the `execute()` and `undo()` methods to define the behavior of the command to be executed, which is to remove a show from a given day using the method `remove()` and to undo the remove operation using the method `add()`.
     */
    public Command createRemoveCommand(Day pDay) {
        return new Command() {
            Show show = aShows.get(pDay); // save reference to the show to be removed
            @Override
            /*?
             * Keyword: execute
             * We override the `execute()` method to define the behavior of the command to be executed, which is to remove a show from a given day using the method `remove()`. First, we save a reference to the show to be removed in a local variable `show` so that we can use it to implement the undo functionality and then we remove the show from the program using the method `remove()`.
             */
            public void execute() {
                show = aShows.get(pDay); // save reference to the show to be removed
                remove(pDay);
            }
            ...
            /*?
             * Keyword: undo
             * We override the `undo()` method to define the behavior of the command to be executed, which is undo the remove operation. This is done by adding the show that was removed back to the program using the method `add()` using the reference to the show that was saved in the local variable `show`.
             */
            public void undo() {
                add(show, pDay);
            }
    ...
    /*?
     * Keyword: createClearCommand
     * We define an instance method to act as a command factory. The method `createClearCommand` is used to create a command that clears the program. The method takes no parameters. The method is implemented as an anonymous class that returns a `Command` object which overrides the `execute()` and `undo()` methods to define the behavior of the commands to be executed, which is to clear the program using the method `clear()` and to undo the clear operation. The undo functionality is implemented by making a copy of the map of shows before clearing the program and then restoring the map in Program using the values in the copy.
     */
    public Command createClearCommand() {

        return new Command() {
            EnumMap<Day, Show> copy = new EnumMap<>(aShows); // save a copy of the map of shows before clearing the program
            @Override
            /*?
             * Keyword: execute
             * We override the `execute()` method to define the behavior of the command to be executed, which is to clear the program using the method `clear()`.
             */
            public void execute() {
                //save a copy of the map of shows before clearing the program
                copy = new EnumMap<>(aShows);
            ...
            /*?
             * Keyword: undo
             * We override the `undo()` method to define the behavior of the command to be executed, which is to undo the clear operation. This is done by restoring the map in Program using the values in the copy.
             */
            public void undo() {
                aShows = copy;
            }
    ...
    public static void main(String[] args) {
        /*?
         * Keyword: program
         * We create a new program object.
         */
        Program program = new Program();

        /*?
         * Keyword: createAddCommand
         * We create a command object to add a movie to Monday using the createAddCommand method by passing a movie and the day MONDAY as parameters to the method and then calling the execute method on the returned command object to execute the command. The movie is added to the program using the add method.
         */
