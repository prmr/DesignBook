...
The impact of this design on the maintainers of class `Card` is unnecessary complexity. Assuming they don't need to look at the source code, there is *no impact* on the users of the class, because the new implementation respects the original interface.
...
Assuming that `Rank` and `Suit` both define a method [`next()`](https://codesample.info/samples/essentials/EnumTypes.html#pop-8), one solution is: 
